(function (root, factory) {
  ...
  class GlorbuCE {
    constructor(opts = {}) {
      ...
      this._knowledge = [];
      this._faq = [];
      this._blocks = []; // sin textos por defecto
    }
    ...
    ask(question) {
      ...
      for (const re of this._forbid) {
        try {
          if (re instanceof RegExp && re.test(q)) {
            const block = this._blocks.length ? pick(this.rng, this._blocks) : "";
            return {
              answer: block,
              intent: "blocked",
              confidence: 1,
          ...
          if (detected) {
            ...
            if (out && typeof out.answer === "string") {
              ...
              for (const t of this._transforms) {
                try {
                  text = t(Object.assign({}, ctx, { intent: intent.name }), text);
                } catch (_e) {}
              }
              text = sanitize(text);
              return {
      ...
      let text = this._openings.length
        ? compose(this.opening(), "", this.closing())
        : "";
      for (const t of this._transforms) {
        try {
          text = t(ctx, text);
      ...
      text = sanitize(text);
      return {
        answer: text, // cadena vacía si no hay addons que provean contenido
        intent: "open",
        confidence: 0.5,
        flags: ["fallback"],
      };
